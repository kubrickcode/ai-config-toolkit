...
아키텍처 또는 설계 결정을 내리기 전에 `docs/decisions/`의 기존 ADR을 스캔한다:

1. **파일명 목록 확인** — 슬러그로 잠재적 관련 ADR 식별
2. **후보 파일만 읽기** — 슬러그 매칭된 ADR의 프론트매터(`status`, `name`, `description`) 확인
3. **건너뛰기** — status가 `폐기됨` 또는 `대체됨`인 ADR 제외
4. **본문 읽기** — 확인된 관련 ADR만 전체 내용 읽기

모든 ADR 파일을 일괄로 읽지 말 것. 관련 ADR이 있으면:
