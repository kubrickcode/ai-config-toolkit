...
1. **설정 파일 먼저 읽기** - `playwright.config.ts`와 `package.json`에서 추출:
   - Base URL과 포트 (webServer.url 또는 use.baseURL)
   - 테스트 디렉토리 (testDir 또는 testMatch)
   - 프레임워크 (package.json dependencies: next, react, vue 등)

2. **브라우저 탐색** - Playwright MCP로 실제 테스트 (코드 분석만으로는 UX 이슈 놓침)

3. **새 문서 생성** - 기존 문서 삭제 후 새로 생성. 과거 내용과 병합하지 않음.

4. **기존 테스트 검증** - 테스트 파일 케이스(describe/test/it) 읽어 중복 방지
